...
Each song includes genre, mood, energy, tempo, valence, danceability, and acousticness. The user profile stores a preferred genre, preferred mood, target energy level, and whether the user likes more acoustic songs. The recommender gives each song points for matching genre and mood, then adds a similarity score based on how close the song's energy is to the user's target. It also adds a small bonus or penalty based on acousticness. After scoring every song, it sorts them from highest to lowest and returns the top results.
